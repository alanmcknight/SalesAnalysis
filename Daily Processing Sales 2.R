...
PolicySales$TrafficCost[grepl("APRQZ-", PolicySales$ECWebref, ignore.case=FALSE) & PolicySales$BTXPoltype == "HQ" & (PolicySales$HWBuildingssi == 0 | PolicySales$HWContentssi == 0) & as.Date(PolicySales$BTXDatecreated, "%d/%m/%Y") > "2017-02-08"] <- 36.50
...
Executive = c("Mark", "Audrey", "Aine", "Louise", "Megan", "Elaine", "Susan", "Stephen")
...
PolicySales$Week.of.Year = week(as.Date(PolicySales$BTXDatecreated, "%d/%m/%Y"))

#Month Created Calculation
#PolicySales$Month1 = month((as.Date(PolicySales$BTXDatecreated, "%d/%m/%Y")))
PolicySales$Month1 = strftime(as.Date(PolicySales$BTXDatecreated, "%d/%m/%Y"), "%y.%m")
PolicySales$Month2 = substr(months(as.Date(PolicySales$BTXDatecreated, "%d/%m/%Y")), 1, 3)
...
PolicySales$Year = as.numeric(format(as.Date(PolicySales$BTXDatecreated, "%d/%m/%Y"),'%Y')) 
...
DailySales$TrafficCost[grepl("APR-", DailySales$ECWebref, ignore.case=FALSE) & DailySales$BTXPoltype == "HQ" & (DailySales$HWBuildingssi == 0 | DailySales$HWContentssi == 0) & as.Date(DailySales$BTXDatecreated, "%d/%m/%Y") > "2017-02-08" & DailySales$QUOTE.REFERENCE != "NA"] <- 36.50
...
Master$BTXDatecreated.y <- as.Date(Master$BTXDatecreated.y, "%d/%m/%Y")
...
Endorsements <- Endorsements[as.Date(Endorsements$BTXDatecreated.x, "%d/%m/%Y") > as.Date(Endorsements$BTXDatecreated.y, "%d/%m/%Y"), ]
Endorsements <- Endorsements[rev(order(as.Date(Endorsements$BTXDatecreated.y, format="%d/%m/%Y"))),]
